...
@Import(BenchmarkControllerTest.TestConfig.class)
class BenchmarkControllerTest {
    ...
    @Test
    void testBenchmarkWithStandardAndPremiumKeys() throws Exception {
        when(rateLimiterService.isAllowed(anyString(), anyInt())).thenReturn(true, false, true, false, true);

        BenchmarkRequest request = new BenchmarkRequest(4, 8L, 2, 1);
        request.setStandardApiKey("api-key-1");
        request.setPremiumApiKey("premium-key-123");
        ...
        assertTrue(benchmarkResponse.isSuccess());
        assertFalse(benchmarkResponse.getPerKeyResults().isEmpty());
        assertTrue(benchmarkResponse.getPerKeyResults().containsKey("api-key-1"));
        assertTrue(benchmarkResponse.getPerKeyResults().containsKey("premium-key-123"));
        assertEquals("STANDARD", benchmarkResponse.getPerKeyResults().get("api-key-1").getUserType());
        assertEquals("PREMIUM", benchmarkResponse.getPerKeyResults().get("premium-key-123").getUserType());
    }
    ...
    void testPerformanceTarget() throws Exception {
        ...
        benchmarkResponse.meetsPerformanceTarget(1000.0); // 1000 req/sec
