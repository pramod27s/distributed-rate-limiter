...
public class RateLimitConfigControllerTest {
    ...
    void test_shouldUpdateDefaultConfiguration() throws Exception {
        ...
        request.setAlgorithm(RateLimitAlgorithm.LEAKY_BUCKET);
        ...
        verify(configuration).setAlgorithm(RateLimitAlgorithm.LEAKY_BUCKET);
    }

    @Test
    void test_shouldReturnGlobalAlgorithmInConfigurationResponse() throws Exception {
        when(configuration.getCapacity()).thenReturn(10);
        when(configuration.getRefillRate()).thenReturn(2);
        when(configuration.getCleanupIntervalMs()).thenReturn(60000L);
        when(configuration.getAlgorithm()).thenReturn(RateLimitAlgorithm.FIXED_WINDOW);

        mockMvc.perform(get("/api/ratelimit/config"))
                .andExpect(status().isOk())
