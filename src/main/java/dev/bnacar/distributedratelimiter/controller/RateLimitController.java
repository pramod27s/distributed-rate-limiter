...
public class RateLimitController {
    ...
    public RateLimitController(RateLimiterService rateLimiterService,
                              ...
                              @org.springframework.beans.factory.annotation.Autowired(required = false) DistributedRateLimiterService distributedRateLimiterService,
                              @org.springframework.beans.factory.annotation.Autowired(required = false) MetricsService metricsService) {
    ...
    public ResponseEntity<RateLimitResponse> checkRateLimit(
        ...
        // Use API key identity when present so premium/standard plans can have different limits.
        String effectiveKey = StringUtils.hasText(request.getApiKey())
            ? request.getApiKey().trim()
            : ipSecurityService.createIpBasedKey(request.getKey(), clientIp);
        String userType = apiKeyService.getApiKeyTier(request.getApiKey());
        
        // Standard single-algorithm rate limiting
        // Prefer distributed Redis-backed limiter when enabled and available.
        boolean allowed = distributedRateLimiterService != null
            ? distributedRateLimiterService.isAllowed(effectiveKey, request.getTokens())
            : rateLimiterService.isAllowed(effectiveKey, request.getTokens());
        
        recordRequestEvent(request, userType, allowed, request.getAlgorithm() != null
    ...
    private void recordRequestEvent(RateLimitRequest request, String userType, boolean allowed, String algorithm) {
        if (metricsService == null) {
            return;
        }

        metricsService.recordRequestEvent(
            request.getKey(),
            request.getApiKey(),
            userType,
            algorithm,
