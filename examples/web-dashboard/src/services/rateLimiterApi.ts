...
interface RateLimiterConfig {
  ...
  algorithm?: string;
  keyConfigs: Record<string, { capacity: number; refillRate: number; algorithm?: string; displayName?: string; description?: string }>;
  patternConfigs: Record<string, { capacity: number; refillRate: number; algorithm?: string; displayName?: string; description?: string }>;
}

interface DefaultConfigRequest {
  capacity?: number;
  refillRate?: number;
  cleanupIntervalMs?: number;
  algorithm?: string;
...
interface ManagedApiKeysResponse {
  keys: ManagedApiKey[];
  totalKeys: number;
  activeKeys: number;
}
...
interface PerKeyLoadTestResult {
  userType: string;
  totalRequests: number;
  successfulRequests: number;
...
class RateLimiterApiService {
  private async request<T>(
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (errorContentType.toLowerCase().includes('json')) {
          const error: ErrorResponse = await response.json();
  ...
  async updateDefaultConfig(config: DefaultConfigRequest): Promise<void> {
  ...
  async getApiKeys(): Promise<ManagedApiKeysResponse> {
    return this.request<ManagedApiKeysResponse>('/api/keys');
  }

  async activateApiKey(key: string): Promise<void> {
    await this.request(`/api/keys/${encodeURIComponent(key)}/activate`, {
      method: 'POST',
    });
  }

  async deactivateApiKey(key: string): Promise<void> {
    await this.request(`/api/keys/${encodeURIComponent(key)}/deactivate`, {
      method: 'POST',
    });
  }
  ...
  async runLoadTest(request: LoadTestRequest, signal?: AbortSignal): Promise<LoadTestResponse> {
...
export type {
  ...
  ApiKeyUsage,
  ManagedApiKey,
  ManagedApiKeysResponse,
  LoadTestRequest,
  PerKeyLoadTestResult,
  LoadTestResponse,
  KeyMetric,
  RequestEvent,
  SystemMetrics,
