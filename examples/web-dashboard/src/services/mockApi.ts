...
interface MockConfigurationPayload {
  global: {
    capacity: number;
    refillRate: number;
    algorithm: string;
...
interface MockApiKeyCreateData {
  name?: string;
  description?: string;
}

interface MockLoadTestConfig {
  targetKey?: string;
  requestRate?: number;
  duration?: number;
  concurrency?: number;
...
export class MockApiService {
  ...
  static async updateConfiguration(config: MockConfigurationPayload) {
    await randomDelay();
    if (shouldSimulateError()) {
  ...
  static async createApiKey(keyData: MockApiKeyCreateData) {
    await randomDelay();
    if (shouldSimulateError()) {
...
export class MockWebSocketService {
  private listeners: Set<(data: MockRealtimeUpdate) => void> = new Set();
  ...
  connect() {
    ...
    this.intervalId = setInterval(() => {
      const update: MockRealtimeUpdate = {
        type: 'metrics_update',
        data: {
          requestsPerSecond: Math.floor(Math.random() * 500) + 1000,
          successRate: Math.floor(Math.random() * 5) + 95,
  ...
  subscribe(listener: (data: MockRealtimeUpdate) => void) {
