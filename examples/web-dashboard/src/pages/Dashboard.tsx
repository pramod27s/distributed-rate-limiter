...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    setTimeSeriesData((prev) => {
      ...
      return [...prev, newPoint].slice(-10); // Keep last 10 data points
    });
    ...
    // Use backend request events so Postman/API traffic is visible in the activity feed.
    const newActivities: ActivityEvent[] = (realtimeMetrics.recentEvents || [])
      .map((event) => ({
        id: event.id,
        ...
        status: event.allowed ? ("allowed" as const) : ("rejected" as const),
        tokensUsed: event.tokensRequested,
      }))
      .slice(0, 20);
      ...
      <Card className="p-6 shadow-elevated backdrop-blur-sm bg-gradient-to-br from-card to-card/50">
        <div className="mb-6 flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">Last 10 samples • Updates every 5 seconds</p>
          </div>
