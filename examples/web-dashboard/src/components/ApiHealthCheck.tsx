  ...
  useEffect(() => {
    const checkHealth = async () => {
      try {
        ...
        if (health.status === 'UP') {
          setStatus('healthy');
          // Safely access redis status component which might be missing if details are not exposed
          if (health.components?.redis) {
            setRedisStatus(health.components.redis.status);
          } else {
             // Fallback if detailed health is not exposed but overall status is UP
            setRedisStatus('UP');
          }
        } else {
          setStatus('error');
